...
#define ST7789_RST_GPIO_CLK_ENABLE() __HAL_RCC_GPIOD_CLK_ENABLE()
...
#define ST7789_CS_LOW()
#define ST7789_CS_HIGH()
...
const static uint16_t
  initlst[] = {
    ...
    ST_CMD_DELAY | 50,             //  150 ms delay shortened to 50
    ST77XX_SLPOUT | ST_CMD ,        //  2: Out of sleep mode, no args, w/delay
	ST_CMD_DELAY | 50,             //  500 ms delay shortened to 50
...
void ST7789_SPI_Init(void)
{
	...
	while(1)
	{
		//Shared_SPI_WriteByte(0xAA);
		Shared_SPI_WriteWord(0xAA55);
	}
#endif
}
